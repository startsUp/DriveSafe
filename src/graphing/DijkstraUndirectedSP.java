...
public class DijkstraUndirectedSP {
     ...
     */
    public DijkstraUndirectedSP(Graph graph, int s) {
 
    	int nodes = graph.V();
        distTo = new double[nodes];
        edgeTo = new Edge[nodes];
        ...
        pq = new IndexMinPQ<>(nodes);
        pq.insert(s, distTo[s]);
        while (!pq.isEmpty()) {
            int v = pq.delMin();
            for (Edge e : graph.getVertex(v).getE())
                relax(e, v);
